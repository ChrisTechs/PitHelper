/*
 * PitHelper - A Hypixel The Pit Helper Mod.
 * Copyright (C) 2026 Christian Steenkamp
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */

package io.github.christechs.pithelper.utils;

import io.github.christechs.pithelper.data.HypixelGameMode;
import net.hypixel.data.type.ServerType;
import net.minecraftforge.fml.common.eventhandler.SubscribeEvent;
import net.minecraftforge.fml.common.network.FMLNetworkEvent;

import java.util.Optional;

public class ServerState {
    public static boolean onHypixel = false;
    public static HypixelGameMode currentGameMode = HypixelGameMode.UNKNOWN;

    public static void updateFromPacket(Optional<ServerType> serverType, Optional<String> lobbyName) {
        onHypixel = true;
        currentGameMode = HypixelGameMode.fromPacket(
                serverType.isPresent() ? serverType.get().name() : "",
                lobbyName.orElse("")
        );
    }

    @SubscribeEvent
    public void onDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        onHypixel = false;
        currentGameMode = HypixelGameMode.UNKNOWN;
    }
}